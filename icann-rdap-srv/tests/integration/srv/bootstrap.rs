...
use icann_rdap_srv::storage::{
    data::{AutnumId, DomainId, NetworkId, NetworkIdType},
    StoreOps,
};
...
#[tokio::test]
async fn GIVEN_bootstrap_with_less_specific_ip_WHEN_query_ip_THEN_status_code_200() {
    // GIVEN
    let test_srv = SrvTestJig::new_bootstrap();
    let mut tx = test_srv.mem.new_tx().await.expect("new transaction");
    tx.add_network_err(
...
#[tokio::test]
async fn GIVEN_bootstrap_with_less_specific_autnum_WHEN_query_autnum_THEN_status_code_200() {
    // GIVEN
    let test_srv = SrvTestJig::new_bootstrap();
    let mut tx = test_srv.mem.new_tx().await.expect("new transaction");
    tx.add_autnum_err(
