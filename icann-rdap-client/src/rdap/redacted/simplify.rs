...
fn simplify_registrant_name(mut domain: Box<Domain>) -> Box<Domain> {
    if let Some(entities) = &mut domain.object_common.entities {
        for entity in entities.iter_mut() {
            if entity.is_entity_role(&EntityRole::Registrant.to_string()) {
...
fn simplify_tech_name(mut domain: Box<Domain>) -> Box<Domain> {
    if let Some(entities) = &mut domain.object_common.entities {
        for entity in entities.iter_mut() {
            if entity.is_entity_role(&EntityRole::Technical.to_string()) {
                let contact = entity.contact();
...
mod tests {
    use super::{
        add_remark, is_only_pre_path, simplify_registrant_name, simplify_registry_domain_id,
        simplify_registry_registrant_id, simplify_registry_tech_id, simplify_tech_name,
        REDACTED_ID, REDACTED_ID_DESC, REDACTED_NAME, REDACTED_NAME_DESC,
    };
    use icann_rdap_common::prelude::{Contact, Domain, Entity, EntityRole, Remark};
