        ...
        if let Some(r_type) = redaction.name().type_field() {
            ...
            if let Ok(registered_redaction) = r_name {
                domain = match registered_redaction {
                    ...
                    RedactedName::RegistryRegistrantId => todo!(),
                    ...
                    RedactedName::RegistryTechId => todo!(),
...
mod tests {
    ...
    fn test_add_remark_with_none_remarks() {
        ...
        assert_eq!(
            result_vec[0].description.as_ref().unwrap().vec().get(0),
            Some(&desc.to_string())
        );
    }
    ...
    fn test_add_remark_with_existing_different_key() {
        ...
        assert_eq!(
            result_vec[1].description.as_ref().unwrap().vec().get(0),
            Some(&desc.to_string())
        );
    }
    ...
    fn test_add_remark_with_multiple_existing_remarks_no_duplicate() {
        ...
        assert_eq!(
            result_vec[2].description.as_ref().unwrap().vec().get(0),
            Some(&desc.to_string())
        );
    }
