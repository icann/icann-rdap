...
mod tests {
    ...
    #[rstest]
    #[case(CacheData::now().cache_control("max-age=0").build(), 100, true)]
    #[case(CacheData::now().cache_control("max-age=100").build(), 0, true)]
    #[case(CacheData::now().cache_control("max-age=100").build(), 50, false)]
    #[case(CacheData::now().build(), 0, true)]
    #[case(CacheData::now().build(), 100, false)]
    #[case(CacheData::now().expires(Utc::now().to_rfc2822()).build(), 100, true)]
    #[case(CacheData::now().expires((Utc::now() + Duration::seconds(50)).to_rfc2822()).build(), 100, false)]
    #[case(CacheData::now().expires((Utc::now() + Duration::seconds(100)).to_rfc2822()).build(), 50, false)]
    #[case(CacheData::now().cache_control("max-age=100").expires(Utc::now().to_rfc2822()).build(), 100, false)]
    #[case(CacheData::now().cache_control("max-age=0").expires((Utc::now() + Duration::seconds(50)).to_rfc2822()).build(), 100, true)]
    ...
    #[rstest]
    #[case(CacheData::now().cache_control("no-cache").build(), false)]
    #[case(CacheData::now().cache_control("no-store").build(), false)]
    #[case(CacheData::now().cache_control("max-age=40").build(), true)]
    ...
    fn GIVEN_data_and_data_cache_WHEN_to_lines_THEN_format_correct() {
        // GIVEN
        let data = "foo";
        let cache_data = CacheData::now().content_length(14).build();
